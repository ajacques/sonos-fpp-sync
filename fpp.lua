...
local ping_hardware_type = ProtoField.uint8("fpp_multisync.ping.hardware_type", "App/Hardware Type", base.HEX)
local major_version = ProtoField.uint16("fpp_multisync.ping.major_version", "Major Version", base.DEC)
local minor_version = ProtoField.uint16("fpp_multisync.ping.minor_version", "Minor Version")
local version_str = ProtoField.string("fpp_multisync.ping.version", "Version")
local operating_mode_field = ProtoField.uint8("fpp_multisync.ping.operating_mode", "Operating Mode", base.HEX, operating_modes)
local ip_address_field = ProtoField.ipv4("fpp_multisync.ping.ip_address", "IP Address")
local hostname_field = ProtoField.stringz("fpp_multisync.ping.hostname", "Hostname")
local version_field = ProtoField.stringz("fpp_multisync.ping.version", "Version")
local hardware_type_field = ProtoField.stringz("fpp_multisync.ping.hardware_type_string", "Hardware Type String")
local channel_ranges_field = ProtoField.stringz( "fpp_multisync.ping.channel_ranges", "Channel Ranges")

local f_sync_action = ProtoField.uint8("fpp_multisync.multisync.action", "MultiSync Action", base.DEC, multisync_actions)
local f_sync_type = ProtoField.uint8("fpp_multisync.multisync.type", "Sync Type", base.HEX, multisync_types)
local f_sync_filename = ProtoField.stringz("fpp_multisync.multisync.file", "File Name")
local f_frame_number = ProtoField.uint32("fpp_multisync.multisync.frame_number", "Frame Number")
local f_seconds = ProtoField.float("fpp_multisync.multisync.seconds", "Seconds Elapsed")
